...
public static class AccountingCompletionEndpoints
{
    public static IEndpointRouteBuilder MapAccountingCompletionEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        group.MapPost("/fiscal-years/{id:guid}/reopen", ReopenFiscalYear).RequireAuthorization(SecurityPolicies.CompanyAdministration);
        group.MapGet("/posting-profiles", (LondonVIPDbContext db, ICompanyContext company, CancellationToken token) => db.AccountingPostingProfiles.AsNoTracking().Where(x => x.CompanyId == company.CompanyId).OrderBy(x => x.EventType).ToListAsync(token));
        group.MapPut("/posting-profiles/{eventType}", UpsertPostingProfile).RequireAuthorization(SecurityPolicies.CompanyAdministration);
        ...
        group.MapGet("/admin/{resource}", AdminQuery);
        group.MapGet("/admin/{resource}/{id:guid}", AdminGet);
        group.MapGet("/admin/{resource}/{id:guid}/history", (string resource, Guid id, IFinanceAdministrationService service, CancellationToken token) => service.HistoryAsync(resource, id, token));
        group.MapPost("/admin/{resource}", AdminCreate);
        group.MapPut("/admin/{resource}/{id:guid}", AdminUpdate);
        group.MapPost("/admin/{resource}/bulk/{action}", AdminBulk);
        return endpoints;
    }
    ...
    static async Task<IResult> ApproveCreditNote(Guid id, ICreditNoteService service, IAuditService audit, ICompanyContext company, IBusinessEventPublisher events, CancellationToken token) => await Execute(async () => { var value = await service.ApproveAsync(id, token); if (value is null) return Results.NotFound(); await WriteAudit(audit, company, "CreditNoteApproved", id, token); await events.PublishAsync(new(BusinessEventTypes.CreditNoteApproved,"CreditNote",value.Id,System.Text.Json.JsonSerializer.Serialize(new{value.TotalAmount}),value.Id.ToString()),token); return Results.Ok(CreditNoteResult(value)); });
    ...
    static async Task<IResult> CreateSupplierPayment(SupplierPaymentRequest request, ISupplierPaymentService service, IAuditService audit, ICompanyContext company, IBusinessEventPublisher events, CancellationToken token) => await Execute(async () => { var value = await service.CreateAsync(request, token); await WriteAudit(audit, company, "SupplierPaymentCreated", value.Id, token); await events.PublishAsync(new(BusinessEventTypes.SupplierPaymentCreated,"SupplierPayment",value.Id,System.Text.Json.JsonSerializer.Serialize(new{value.Amount}),value.Id.ToString()),token); return Results.Created($"/api/finance/supplier-payments/{value.Id}", value); });
    ...
    static async Task<IResult> ReopenFiscalYear(Guid id, string correlationId, IFiscalPeriodService service, IAuditService audit, ICompanyContext company, CancellationToken token) => await Execute(async () => { if (!await service.ReopenYearAsync(id, correlationId, token)) return Results.NotFound(); await WriteAudit(audit, company, "FiscalYearReopened", id, token); return Results.Ok(); });
    static async Task<IResult> UpsertPostingProfile(string eventType, PostingProfileRequest request, LondonVIPDbContext db, ICompanyContext company, IAuditService audit, CancellationToken token)
    {
        if (!await db.LedgerAccounts.AnyAsync(x => x.CompanyId == company.CompanyId && x.Id == request.DebitAccountId && x.IsActive, token) || !await db.LedgerAccounts.AnyAsync(x => x.CompanyId == company.CompanyId && x.Id == request.CreditAccountId && x.IsActive, token)) return Results.BadRequest(new { Message = "Posting accounts are invalid." });
        var value = await db.AccountingPostingProfiles.SingleOrDefaultAsync(x => x.CompanyId == company.CompanyId && x.EventType == eventType, token);
        if (value is null) { value = new AccountingPostingProfile { Id = Guid.NewGuid(), CompanyId = company.CompanyId, EventType = eventType, CreatedAt = DateTimeOffset.UtcNow }; db.Add(value); }
        value.DebitAccountId = request.DebitAccountId; value.CreditAccountId = request.CreditAccountId; value.IsActive = request.IsActive; value.UpdatedAt = DateTimeOffset.UtcNow; await db.SaveChangesAsync(token); await WriteAudit(audit, company, "PostingProfileUpdated", value.Id, token); return Results.Ok(value);
    }
    ...
    static async Task<IResult> CreateBankMatch(BankMatchRequest request, IBankReconciliationService service, IAuditService audit, ICompanyContext company, IBusinessEventPublisher events, CancellationToken token) => await Execute(async () => { var value = await service.MatchAsync(request, token); await WriteAudit(audit, company, "BankMatchCreated", value.Id, token); await events.PublishAsync(new(BusinessEventTypes.BankReconciled,"BankTransactionMatch",value.Id,System.Text.Json.JsonSerializer.Serialize(new{value.Amount}),request.CorrelationId),token); return Results.Created($"/api/finance/bank/matches/{value.Id}", value); });
    ...
    static async Task<IResult> FinancePdf(string report, DateOnly from, DateOnly to, IAccountingReportService accounting, IFinancialStatementService statements, IPdfGenerationProvider pdf, CancellationToken token)
    {
        ...
        switch (normalized) { case "balance-sheet": model = await statements.BalanceSheetAsync(to, from.AddYears(-1), token); type = PdfDocumentType.BalanceSheet; break; case "profit-loss": model = await accounting.ProfitAndLossAsync(from, to, token); type = PdfDocumentType.ProfitAndLoss; break; case "cash-flow": model = await statements.CashFlowAsync(from, to, token); type = PdfDocumentType.CashFlow; break; case "vat" or "vat-return": model = await accounting.VatAsync(from, to, token); type = PdfDocumentType.VatReport; break; default: var types = new Dictionary<string,PdfDocumentType>{{"trial-balance",PdfDocumentType.TrialBalance},{"general-ledger",PdfDocumentType.GeneralLedger},{"journals",PdfDocumentType.Journal},{"journal-listing",PdfDocumentType.Journal},{"supplier-statement",PdfDocumentType.SupplierStatement},{"customer-statement",PdfDocumentType.CustomerStatement},{"driver-settlement",PdfDocumentType.DriverSettlement},{"expense-analysis",PdfDocumentType.ExpenseAnalysis},{"revenue-analysis",PdfDocumentType.RevenueAnalysis},{"budget-report",PdfDocumentType.BudgetReport},{"aged-debtors",PdfDocumentType.AgedDebtors},{"aged-creditors",PdfDocumentType.AgedCreditors}};if(!types.TryGetValue(normalized,out type))return Results.BadRequest(new{Message="Unsupported PDF report."});var export=await statements.ExportAsync(normalized,"csv",from,to,token);model=new{Report=normalized,From=from,To=to,Data=System.Text.Encoding.UTF8.GetString(export.Content)};break; }
    ...
    static Task<FinanceAdminPage> AdminQuery(string resource, string? search, string? sort, bool descending, int page, int pageSize, bool includeArchived, IFinanceAdministrationService service, CancellationToken token) => service.QueryAsync(resource, new(search, sort, descending, page, pageSize, includeArchived), token);
    static async Task<IResult> AdminGet(string resource, Guid id, IFinanceAdministrationService service, CancellationToken token) { var value = await service.GetAsync(resource, id, token); return value.HasValue ? Results.Ok(value.Value) : Results.NotFound(); }
    static async Task<IResult> AdminCreate(string resource, System.Text.Json.JsonElement value, HttpContext http, IFinanceAdministrationService service, IAuditService audit, ICompanyContext company, CancellationToken token) => await Execute(async () => { var result = await service.CreateAsync(resource, value, http.TraceIdentifier, token); var id = result.GetProperty("id").GetGuid(); await WriteAudit(audit, company, "FinanceRecordCreated", id, token); return Results.Created($"/api/finance/admin/{resource}/{id}", result); });
    static async Task<IResult> AdminUpdate(string resource, Guid id, System.Text.Json.JsonElement changes, HttpContext http, IFinanceAdministrationService service, IAuditService audit, ICompanyContext company, CancellationToken token) => await Execute(async () => { var result = await service.UpdateAsync(resource, id, changes, http.TraceIdentifier, token); if (!result.HasValue) return Results.NotFound(); await WriteAudit(audit, company, "FinanceRecordUpdated", id, token); return Results.Ok(result.Value); });
    static async Task<IResult> AdminBulk(string resource, string action, FinanceBulkRequest request, HttpContext http, IFinanceAdministrationService service, IAuditService audit, ICompanyContext company, CancellationToken token) => await Execute(async () => { var count = await service.BulkAsync(resource, action, request, http.TraceIdentifier, token); await WriteAudit(audit, company, $"FinanceBulk{action}", Guid.Empty, token); return Results.Ok(new { count }); });
    ...
    public sealed record PostingProfileRequest(Guid DebitAccountId, Guid CreditAccountId, bool IsActive);
}
