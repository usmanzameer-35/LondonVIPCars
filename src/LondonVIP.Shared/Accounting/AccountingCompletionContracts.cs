...
public interface IFiscalPeriodService { Task<FiscalYear> CreateAsync(FiscalYearRequest request,CancellationToken token=default); Task<bool> SetPeriodStatusAsync(Guid periodId,bool close,CancellationToken token=default); Task<bool> CloseYearAsync(Guid fiscalYearId,CancellationToken token=default); }
